...
{{define "marshalScalarElement"}}
	...
	{{if .IsCustomElementEncoder}}
		data, err := {{.CustomElementEncoder}}.MarshalBinary(({{.PointerDeref}}{{.Var}}))
		if err != nil {
	...
	{{else if .IsStruct}}
					nestedData, err := {{.Var}}.MarshalBinary()
					if err != nil {
						return nil, fmt.Errorf("failed to marshal {{.FieldName}}: %v", err)
					}
...
{{define "marshalSlice"}}
{{if .IsCustomElementEncoder }}
	data, err := {{.CustomElementEncoder}}.MarshalBinary(({{.PointerDeref}}s.{{.Field.Name}}))
		if err != nil {
...
{{else}}
	// Direct encoding
	d, _ := (s.{{.Field.Name}}).MarshalBinary()
...
{{if .Shape.IsCustomElementEncoder}}
		data, err := {{.Shape.CustomElementEncoder}}.MarshalBinary(({{.Shape.PointerDeref}}{{.Var}}))
		if err != nil {
			return nil, fmt.Errorf("failed to marshal {{.FieldName}}: %v", err)
		}
		buf = appendBytes(buf, data)
