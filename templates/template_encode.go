...
{{define "encodeScalarElement"}}
	...
	{{if .IsCustomElementEncoder}}
		data, err := {{.CustomElementEncoder}}.Encode(({{.PointerDeref}}{{.Var}}))
		if err != nil {
...
{{define "encodeSlice"}}
{{if .IsCustomElementEncoder }}
	data, err := {{.CustomElementEncoder}}.Encode(({{.PointerDeref}}s.{{.Field.Name}}))
		if err != nil {
...
{{if .Shape.IsCustomElementEncoder}}
		data, err := {{.Shape.CustomElementEncoder}}.Encode(({{.Shape.PointerDeref}}{{.Var}}))
		if err != nil {
			return nil, fmt.Errorf("failed to encode {{.FieldName}}: %v", err)
		}
		buf = append(buf, data...)
